    ...
    module.OrderWidget = module.PosBaseWidget.extend({
        ...
        init: function(parent, options) {
            ...
            this.line_click_handler = function(event){
                ...
                if ( self.pos_widget.screen_selector.get_current_screen() !== "products" ) {
                    self.pos_widget.screen_selector.set_current_screen('products');
                    self.pos_widget.product_options_widget.reset();
                } else {
        ...
        update_summary: function(){
            ...
            this.el.querySelector('.order-summary .total > .subentry-total > .value').textContent = total;
    ...
    module.ProductListWidget = module.PosBaseWidget.extend({
        ...
        set_deselected_product: function(){
            if(this.selected_product_id){
                var querySelector = this.el.querySelector("[data-product-id='"+this.selected_product_id+"'] > .product-img");
                if(querySelector){
                    querySelector.className = 'product-img';
        ...
        set_template:function(product_template){
            ...
            if(product_template === this.selected_template){
                ...
                if (product_template.attribute_line_ids.length === 0) {
                    ...
                    if(querySelector){
                        querySelector.value = this.total_quantity;
                    }
                    return;
                }
                ...
                setTimeout(function(){
                    self.renderElement();
                    self.show();
                    self.el.querySelector(".product-header > .product-name").textContent = self.selected_template.display_name;
                }, 450);
            }
        },
